...
This refines [the three-person roadmap](product.md#three-person-parallel-mvp-roadmap): **A** owns profile evidence/corrections and truthful résumé suggestions; **B** owns path taxonomy, reviewed checklist definitions, progress/tree UI, job links and contextual learning/community resources; **C** owns authenticated persistence, navigation, accessible shared components and deployment. All three agree checkpoint states, versioning and saved-action contracts in M0. Existing API/schema proposals need those contracts added before implementation; this story does not pretend they already exist.
