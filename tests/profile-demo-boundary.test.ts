...
test("live demo embeds only approved summaries; HTTP corrections stay simulated", async () => {
  ...
  try {
        ...
        candidates: [
          {
            ...
            content: {
              parts: [
                { text: JSON.stringify({ facts: resumeFixtures[0].facts }) },
              ],
            },
          },
        ],
    ...
    const intake = await handlers.post(
      request("POST", { text: resumeFixtures[0].text }),
    );
    assert.equal(intake.status, 201);
    const profile = await intake.json();
    const corrections = profile.facts.map(
      ({ id, kind, label, detail, dateText }: Record<string, unknown>) => ({
        id,
        kind,
        label,
        detail,
    ...
    const approved = await handlers.patch(
      request("PATCH", { expectedVersion: 1, confirm: true, corrections }),
      profile.profileId,
    );
    assert.equal(approved.status, 200);
    assert.equal((await approved.json()).embedding.simulated, false);
    assert.deepEqual(embedded, [
      resumeFixtures[0].facts.map(factLine).join("\n"),
    ]);
    corrections[0].label = "Arbitrary correction outside the approved fixtures";
    const edited = await handlers.patch(
      request("PATCH", { expectedVersion: 2, confirm: true, corrections }),
      profile.profileId,
    );
    ...
    assert.equal(
      embedded.length,
      1,
      "edited summary must not reach live embedding",
    );
    const draft = await handlers.patch(
      request("PATCH", { expectedVersion: 3, confirm: false, corrections }),
      profile.profileId,
    );
