...
test("HTTP requires authenticated owner, same origin and idempotency", async () => {
  ...
  assert.equal(
    (await routes.post(post({ origin: "https://attacker.invalid" }))).status,
    403,
  );
  assert.equal(
    (await routes.post(post({ "idempotency-key": "" }))).status,
    400,
  );
});
    ...
    (
      await routes.get(
        new Request("http://localhost", {
          headers: { "x-test-owner": randomUUID() },
        }),
        p.profileId,
      )
    ).status,
...
test("origin checks use the actual Host when Next normalizes loopback URLs", async () => {
  const { checkOrigin } = await import("../src/profile/http");
  const saved = process.env.APP_BASE_URL;
  delete process.env.APP_BASE_URL;
  try {
    assert.doesNotThrow(() =>
      checkOrigin(
        new Request("http://localhost:3101/api/resumes", {
    ...
    assert.throws(
      ...
      (error: unknown) =>
        error instanceof ProfileError && error.code === "INVALID_ORIGIN",
    );
