...
const code = (name: string) => (e: unknown) =>
  e instanceof ProfileError && e.code === name;
...
test("oversize, non-PDF, malformed, too many pages and no-text PDFs fail", async () => {
  await assert.rejects(
    parsePdf(new Uint8Array(2 * 1024 * 1024 + 1)),
    code("FILE_TOO_LARGE"),
  );
  await assert.rejects(
    parsePdf(new TextEncoder().encode("not pdf")),
...
test("JSON input is strict and malformed requests produce useful errors", async () => {
  ...
  assert.equal(
    await readResumeInput(
      request(JSON.stringify({ text: resumeFixtures[0].text })),
    ),
  ...
  await assert.rejects(
    readResumeInput(
      request(JSON.stringify({ text: "sample", ownerId: "forged" })),
    ),
    code("INVALID_INPUT"),
  );
});
test("stream limit works even without Content-Length", async () => {
  await assert.rejects(
    readBoundedBody(
      new Request("http://localhost", { method: "POST", body: "123456789" }),
      5,
    ),
    code("FILE_TOO_LARGE"),
  );
...
test("multipart accepts only a single PDF with matching magic", async () => {
  ...
  form.set(
    "file",
    new File(
      [Buffer.from(syntheticPdf(resumeFixtures[0].text))],
      "sample.pdf",
      {
