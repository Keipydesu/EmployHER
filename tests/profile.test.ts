...
} from "../src/profile/adapters/memory";
  ...
  test(`synthetic extraction: ${fixture.id}`, async () => {
    ...
    if (!fixture.facts.length)
      return assert.rejects(
        service.intake(user, randomUUID(), fixture.text),
        code("NO_EVIDENCE"),
      );
    ...
    for (const fact of p.facts) {
      ...
      if (fact.evidence.source === "resume")
        assert.equal(
          prepareText(fixture.text).slice(
            fact.evidence.start,
            fact.evidence.end,
          ),
...
test("forged evidence, arbitrary owner IDs, unknown and repeated fact IDs are rejected", async () => {
  ...
  await assert.rejects(
    service.update(user, p.profileId, randomUUID(), {
      ...update(p),
      ownerId: randomUUID(),
    }),
  ...
  assert.throws(
    () =>
      applyCorrections(p.facts, [
        { ...update(p).corrections[0], id: randomUUID() },
      ]),
...
test("another owner cannot read, update, or request suggestions", async () => {
  ...
  await assert.rejects(
    service.suggestions(stranger, p.profileId, "data", 1),
    code("NOT_FOUND"),
  );
});
test("idempotency replay returns same profile; changed input conflicts", async () => {
  ...
  assert.equal(
    (await service.intake(user, key, resumeFixtures[0].text)).profileId,
    p.profileId,
  );
  await assert.rejects(
  ...
  assert.deepEqual(
    await service.update(user, p.profileId, updateKey, update(p)),
    next,
  );
});
...
test("invalid dimensions, nonfinite and zero vectors cannot confirm a profile", async () => {
  ...
  for (const values of [
    ...
    Array(768).fill(0),
    Array(768).fill(NaN),
    Array(768).fill(Infinity),
  ])
    assert.throws(
      () => validateEmbedding({ ...valid, values }),
...
test("matching rejects draft, simulated, stale, expired profiles", async () => {
  ...
  const p = await service.intake(user, randomUUID(), resumeFixtures[0].text);
  await assert.rejects(
    service.matchingProfile(user, p.profileId, 1),
    code("CONFIRM_FIRST"),
  );
  await service.update(user, p.profileId, randomUUID(), update(p));
  await assert.rejects(
  ...
  await assert.rejects(
    service.matchingProfile(user, p.profileId, 2),
    code("SIMULATED_VECTOR"),
  );
  const expired = {
    ...p,
...
test("deletion prevents replay and late embedding writes", async () => {
  ...
  await assert.rejects(
    service.intake(user, key, resumeFixtures[0].text),
    code("NOT_FOUND"),
  );
...
test("suggestions preserve confirmed text, require reviewed path, and reject stale profile", async () => {
  ...
  const p = await service.intake(user, randomUUID(), resumeFixtures[0].text);
  await assert.rejects(
    service.suggestions(user, p.profileId, "data", 1),
    code("CONFIRM_FIRST"),
  );
