...
Generation must not run as in-process async work. On request-based billing Cloud Run disables or severely limits CPU outside a request, so a job started in the web process is not guaranteed to finish after the response is sent. Use one durable execution model and only one: a Cloud Tasks queue dispatching to an authenticated Cloud Run endpoint that invokes the generation service. A Solid Queue worker on an always-allocated-CPU service is a valid alternative, but it is a choice between the two, not an addition; two backends means two independent retry budgets and no single owner of correctness. The alternative also carries continuous worker cost against the spend cap.

**Request identity and idempotency.** The unit of work is a `GenerationRequest { id, ownerId, operation: analysis|roadmap|outreach, analysisId?, intendedRoadmapVersion?, state, attempts, createdAt }` persisted before the task is enqueued. Idempotency is keyed on the request ID and operation, not on the analysis ID: a single analysis legitimately produces several operations, and F9's explicit regeneration is *supposed* to create a new roadmap version. A transport redelivery reuses the same request ID and must not produce a second plan; an explicit user regeneration mints a new request ID and a new intended version.

**Authorization in the worker.** The task's service identity authenticates the caller; it does not establish who owns the work. The worker resolves the owner from the stored `GenerationRequest`, never from the task payload and never from a browser cookie, and re-checks that the owner's records still exist before writing. Callbacks arriving after an owner deletion are rejected, not applied.

**Input handoff.** A queued task cannot read the originating request's memory, so inputs must be staged, not held. Write the resume PDF or pasted resume text and the job description to private owner-scoped Cloud Storage objects before enqueueing, and put only opaque request and object identifiers in the task payload. Pasted text is staged the same way as an uploaded file; it is not exempt because it never was a file.

**Two retry budgets, kept apart.** Schema or grounding repair is one internal model retry inside a single attempt and does not reset anything. Transport redelivery by the queue is separate and explicitly bounded, with a per-request deadline and maximum attempt count recorded on the request. The 60-second design budget applies to one attempt, not to the lifetime of a redelivered task. A permanent validation failure is terminal: record the failed state and return a successful transport acknowledgement so the queue stops redelivering a deterministic failure. Only genuinely transient faults are allowed to consume redeliveries.

The client polls for status; F10's processing display reports that real state. Never silently replace a failed live run with a sample result.
...
The application framework is **Ruby on Rails**, set by the operator. Rails supplies the request layer, ActiveRecord persistence, ActiveStorage file handling, and encrypted signed sessions. Background generation deliberately runs outside the web process on Cloud Tasks rather than on an in-process job backend, for the reasons in section 6. The sections below record how each cloud dependency maps onto it.
...
Full raw resumes and full raw text are not persisted in the database. Minimal supporting resume excerpts remain sensitive structured data and follow the same retention policy as profiles. Because generation is asynchronous, the resume text or PDF and the job description are staged as private owner-scoped Cloud Storage objects for the duration of the request rather than existing only in process memory; they are purged on terminal success or failure, and in all cases within 24 hours. Retain only cited requirement excerpts afterward. Outreach drafts may remain local until copied rather than adding another retained record.
...
- Delete the staged inputs — raw PDF, extracted full text, pasted resume text, and the job description object — immediately after successful analysis or terminal failure. A cleanup job removes abandoned staged objects within 24 hours; no real-data pilot without that job. Queue tasks must expire no later than the purge horizon, so a task can never outlive the inputs it references.
...
- Never log raw resume/JD bodies, sensitive excerpts, or generated outreach. Log request IDs, timing, validation failures, and model versions. Task payloads carry opaque request and object identifiers only; no resume or job-description content ever enters a queue payload or a queue log.
