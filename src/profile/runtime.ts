...
const state = globalThis as typeof globalThis & {
  ...
  employherProfileDemo?: { runtime: ProfileRuntime; sessions: Map<string, DemoSession> };
};
...
function demoState() {
  ...
  if (!state.employherProfileDemo) {
    ...
    state.employherProfileDemo = {
      sessions,
      runtime: {
        service: new ProfileService(new MemoryProfileRepository(), ai, new MemoryOperations(), {
        ...
        async authorize(request, operation) {
          ...
          if (!session || session.expires < Date.now())
            throw new ProfileError('UNAUTHENTICATED', 401, 'Start a local sample session first.');
        ...
        async authorizeIntake(_owner, text) {
          ...
          const normalized = prepareText(text).replace(/\s+/g, ' ').replace(/—/g, '-');
          if (
            !resumeFixtures.some(
              (f) => prepareText(f.text).replace(/\s+/g, ' ').replace(/—/g, '-') === normalized,
            )
          ) {
            throw new ProfileError(
              'FIXTURE_ONLY',
...
export function createDemoSession(): string {
  ...
  sessions.set(token, { owner: randomUUID(), expires: Date.now() + 3600_000, writes: 0 });
  return token;
}
