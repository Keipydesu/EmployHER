...
export async function parsePdf(bytes: Uint8Array): Promise<string> {
  if (bytes.length > LIMITS.bytes)
    throw new ProfileError('FILE_TOO_LARGE', 413, 'PDFs must be 2 MB or smaller.');
  ...
  return new Promise((resolve, reject) => {
    ...
    worker.on('error', () => {
      finish(
        new ProfileError('UNREADABLE_PDF', 422, 'This PDF could not be read. Paste text instead.'),
      );
    });
    worker.on('exit', () => {
      if (!settled)
...
export async function readResumeInput(request: Request): Promise<string> {
  ...
  if (type.startsWith('application/json')) {
    ...
    const parsed = z.strictObject({ text: z.string().max(LIMITS.characters) }).safeParse(input);
    if (!parsed.success)
      throw new ProfileError(
        'INVALID_INPUT',
        400,
  ...
  if (type.startsWith('multipart/form-data')) {
    ...
    try {
      ...
      throw new ProfileError('INVALID_FORM', 400, 'The upload could not be read.');
    }
    ...
    if (file.type !== 'application/pdf')
      throw new ProfileError('UNSUPPORTED_FILE', 415, 'Only text-based PDFs are supported.');
    return parsePdf(new Uint8Array(await file.arrayBuffer()));
  }
  throw new ProfileError('UNSUPPORTED_FILE', 415, 'Use JSON text or a multipart PDF upload.');
}
