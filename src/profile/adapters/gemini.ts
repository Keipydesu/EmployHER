...
export class GeminiProfileAI implements ProfileAI {
  constructor(
    ...
    if (!apiKey || !/^[a-zA-Z0-9._-]+$/.test(model) || !/^[a-zA-Z0-9._-]+$/.test(embeddingModel)) {
      throw new ProfileError('AI_NOT_CONFIGURED', 503, 'Gemini has not been configured.');
    }
  }
  private async call(
    model: string,
    ...
    try {
      ...
      if (signal.aborted)
        throw new ProfileError('AI_TIMEOUT', 504, 'The AI request timed out. Please retry.', true);
      throw new ProfileError(
        'AI_UNAVAILABLE',
        502,
  ...
  async extract(text: string, signal: AbortSignal) {
    ...
    const response = await this.call(
      ...
      'generateContent',
      {
        ...
        contents: [{ role: 'user', parts: [{ text: JSON.stringify({ untrustedResume: text }) }] }],
        generationConfig: {
    ...
    const envelope = z
      ...
      .safeParse(response);
    if (!envelope.success || envelope.data.candidates[0].finishReason !== 'STOP') {
      throw new ProfileError(
        'EXTRACTION_BLOCKED',
        502,
  ...
  async embed(summary: string, signal: AbortSignal): Promise<Embedding> {
    ...
    const parsed = z
      .object({
        embedding: z.object({ values: z.array(z.number().finite()).length(LIMITS.dimensions) }),
      })
      .safeParse(response);
    if (!parsed.success)
      throw new ProfileError(
