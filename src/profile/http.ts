...
import { demoEnabled, getProfileRuntime, type ProfileRuntime } from './runtime';

export function checkOrigin(request: Request) {
  const expected = new URL(process.env.APP_BASE_URL || request.url);
  const origin = request.headers.get('origin');
  let localAlias = false;
  if (demoEnabled() && origin) {
    try {
      const supplied = new URL(origin);
      // Next dev canonicalizes 127.0.0.1 to localhost in Request.url.
      localAlias =
        ['localhost', '127.0.0.1'].includes(expected.hostname) &&
        ['localhost', '127.0.0.1'].includes(supplied.hostname) &&
        expected.port === supplied.port &&
        expected.protocol === supplied.protocol;
    } catch {
      /* Invalid Origin is rejected below. */
    }
  }
  if (
    (origin !== expected.origin && !localAlias) ||
    request.headers.get('sec-fetch-site') === 'cross-site'
  ) {
    throw new ProfileError('INVALID_ORIGIN', 403, 'This action must come from the application.');
  }
}
...
export function createProfileHandlers(runtime: () => ProfileRuntime = getProfileRuntime) {
  return {
    post: (request: Request) =>
      respond(async () => {
        ...
        return publicProfile(await dependencies.service.intake(owner, requestKey, text));
      }, 201),
    get: (request: Request, id: string) =>
      respond(async () => {
        const dependencies = runtime();
        const owner = await dependencies.authorize(request, 'read');
        return publicProfile(await dependencies.service.get(owner, profileId(id)));
      }),
    patch: (request: Request, id: string) =>
      respond(async () => {
        ...
        let input: unknown;
        try {
          input = JSON.parse(new TextDecoder().decode(await readBoundedBody(request, 64 * 1024)));
        } catch (error) {
          if (error instanceof ProfileError) throw error;
          throw new ProfileError('INVALID_INPUT', 400, 'Invalid JSON corrections.');
        }
        return publicProfile(
          await dependencies.service.update(owner, profileId(id), requestKey, input),
        );
      }),
