...
Local MVP override: [decision 008](docs/decisions/008-local-personal-resume-demo.md)
allows explicit authenticated personal-upload mode on loopback despite the
unresolved unpaid-provider constraint. References below to keeping intake disabled
remain the default/hosted release policy; enabling this local mode does not close
R3 or prove successful connected acceptance.
...
- Backboard storage: server REST adapter, durable versioned queue (migration008), opt-in/removal controls and cleanup retries are implemented. Saved field/active-action context only; no raw résumé or automatic chat-model call. Account deletion waits for provider cleanup. Two adapter tests and a real-PostgreSQL/fake-provider race test pass; 14 database tests now use isolated schemas. Live Backboard API and provider-tier acceptance remain open, as does authenticated browser coverage.

- Backboard live adapter verification (2026-09-19): temporary synthetic assistant creation and memory create/read/update/read/delete/reset passed against the configured service, with assistant cleanup confirmed. Live testing corrected the create-response schema; focused adapter tests and code checks pass. Peer regression checks cover profile expiry during assistant creation and retry from a pending row with an unknown memory ID (16 database tests). This proves direct provider CRUD, not the complete authenticated worker/UI journey or provider-retention acceptance. The checkpoint PR's browser CI failed on Auth0 initialization from `APP_BASE_URL` alone; a local fix excludes that non-credential from detection, with seven browser tests passing per peer review. That fix is not yet pushed.

- Authenticated onboarding browser harness: two tests now pass through real SDK session validation and PostgreSQL interest persistence in an isolated local schema. They cover two-user isolation, mobile keyboard selection, stale edits, foreign origins and sign-out denial. Desktop/mobile screenshots were inspected. A real stale-edit HTTP 500 revealed cross-bundle error-class identity; shared symbol brands restore the intended 409. Real Auth0 login/callback and the connected résumé-to-career journey remain open; these tests do not call Gemini or use production credentials.

- Career UI now exposes listing/cohort/location preferences, optional community categories, source-backed evidence checkpoints and a coverage progress bar, explicit learning-need confirmation and reviewed resources. A third authenticated browser test verifies these controls plus synthetic provider failure/retry and saved-action interactions. Career API responses in that test are intercepted fixtures; onboarding still uses real PostgreSQL. Code checks and all three browser cases pass; desktop/mobile screenshots inspected. A separate live synthetic Gemini recommendation retry timed out, so no live synthesis success is claimed.

- Account-data controls: `/account` now exposes confirmed deletion and durable pending/failed/completed status. Deleting users are redirected from profile/onboarding to their status page; other users cannot read their deletion record. Four authenticated browser cases and 16 PostgreSQL integration tests pass. Mobile navigation overflow found by the new link was fixed. Provider cleanup timing, actual Auth0 account handling and personal-data release remain separate open gates.

- Repeatable `smoke:services -- --backboard` passed against the configured database and providers: supplied-fixture extraction, real embeddings, durable owned profile reload/isolation, selected-field retrieval, preference persistence, Backboard worker sync/opt-out, and scoped cleanup of both synthetic accounts. `--gemini` separately failed at real extraction with `AI_UNAVAILABLE`; its accounts were cleaned. This closes live service-adapter persistence/memory evidence, not real Auth0 or full HTTP acceptance. Sixteen DB tests and code checks pass; the cleanup filters are tested to preserve unrelated account queues.
